...
decode:
	...
	; C4/C5 are vex-prefixes only if modrm.mod=3 (see les/lds)
	mov	cl,al
	lodsb
	mov	ch,al
	and	ch,0C0h
...
opcode_table:
	...
	db	C_NONE			; 06h - push es
	db	C_NONE			; 07h - pop es
	...
	db	C_NONE			; 16h - push ss
	db	C_NONE			; 17h - pop ss
	...
	db	C_NONE			; 1Eh - push ds
	db	C_NONE			; 1Fh - pop ds
	...
	db	C_PREFIX		; 3Eh - ds:
	db	C_NONE			; 3Fh - aas
	db	C_NONE			; 40h - inc eax
	db	C_NONE			; 41h - inc ecx
	db	C_NONE			; 42h - inc edx
	db	C_NONE			; 43h - inc ebx
	db	C_NONE			; 44h - inc esp
	db	C_NONE			; 45h - inc ebp
	db	C_NONE			; 46h - inc esi
	db	C_NONE			; 47h - inc edi
	db	C_NONE			; 48h - dec eax
	db	C_NONE			; 49h - dec ecx
	db	C_NONE			; 4Ah - dec edx
	db	C_NONE			; 4Bh - dec ebx
	db	C_NONE			; 4Ch - dec esp
	db	C_NONE			; 4Dh - dec ebp
	db	C_NONE			; 4Eh - dec esi
	db	C_NONE			; 4Fh - dec edi
	db	C_NONE			; 50h - push eax
	db	C_NONE			; 51h - push ecx
	db	C_NONE			; 52h - push edx
	db	C_NONE			; 53h - push ebx
	db	C_NONE			; 54h - push esp
	db	C_NONE			; 55h - push ebp
	db	C_NONE			; 56h - push esi
	db	C_NONE			; 57h - push edi
	db	C_NONE			; 58h - pop eax
	db	C_NONE			; 59h - pop ecx
	db	C_NONE			; 5Ah - pop edx
	db	C_NONE			; 5Bh - pop ebx
	db	C_NONE			; 5Ch - pop esp
	db	C_NONE			; 5Dh - pop ebp
	db	C_NONE			; 5Eh - pop esi
	db	C_NONE			; 5Fh - pop edi
	db	C_NONE			; 60h - pushad|pusha
	db	C_NONE			; 61h - popad|popa
	db	C_MODRM 		; 62h - bound r32, m32&32
	db	C_MODRM 		; 63h - arpl r/m16, r16
	db	C_PREFIX		; 64h - fs:
	db	C_PREFIX		; 65h - gs:
	...
	db	C_MODRM 		; 8Fh - pop r/m32	  (if modrm.reg is not zero, xop coding scheme is used)
	...
	db	C_NONE			; 98h - cwde|cbw
	db	C_NONE			; 99h - cdq|cwd
	db	C_IMM32+C_IMM16 	; 9Ah - call ptr16:32
	db	C_NONE			; 9Bh - fwait		  (used as prefix for fstcw, fstenv, fsave, fstsw, flcex and finit)
	db	C_NONE			; 9Ch - pushfd|pushf
	db	C_NONE			; 9Dh - popfd|popf
	db	C_NONE			; 9Eh - sahf		  (only valid if cpuid.80000001h:ecx.lahf-sahf[bit0]=1)
	db	C_NONE			; 9Fh - lahf		  (^)
	db	C_MOFFS 		; A0h - mov al, moffs8
	db	C_MOFFS 		; A1h - mov eax, moffs32
	db	C_MOFFS 		; A2h - mov moffs8, al
	db	C_MOFFS 		; A3h - mov moffs32, eax
	db	C_NONE			; A4h - movsb
	db	C_NONE			; A5h - movsd|movsw
	db	C_NONE			; A6h - cmpsb
	db	C_NONE			; A7h - cmpsd|cmpsw
	db	C_IMM8			; A8h - test al, imm8
	db	C_IMM32 		; A9h - test eax, imm32
	db	C_NONE			; AAh - stosb
	db	C_NONE			; ABh - stosd|stosw
	db	C_NONE			; ACh - lodsb
	db	C_NONE			; ADh - lodsd|lodsw
	db	C_NONE			; AEh - scasb
	db	C_NONE			; AFh - scasd|scasw
	...
	db	C_NONE			; C3h - retn
	db	C_MODRM 		; C4h - les r32, m16:32   (if modrm.mod=3, it's a vex-prefix)
	db	C_MODRM 		; C5h - lds r32, m16:32   (^)
	...
	db	C_IMM32+C_IMM16 	; EAh - jmp ptr16:32
...
opcode_table_0F:
	...
	db	C_GROUP+C_MODRM 	; 01h - sgdt m || vm*	  (also sidt || [modrm.rm=0|1|2|3] monitor|mwait|clac|stac, lgdt m16&32 || [modrm.rm=0|1|4|5|6] xgetbv|xsetbv|vmfunc|xend|xtest, lidt m16&32, smsw r32/m16, <>, lmsw r/m16, invlpg m || [modrm.rm=0|1] swapgs|rdtscp) *[modrm.rm=1|2|3|4] vmcall|vmlaunch|vmresume|vmxoff
	db	C_MODRM 		; 02h - lar r32, r32/m16
	db	C_MODRM 		; 03h - lsl r32, r32/m16
	db	C_UNDEFINED		; 04h
	db	C_NONE			; 05h - syscall
	db	C_NONE			; 06h - clts
	db	C_NONE			; 07h - sysret
	db	C_NONE			; 08h - invd
	...
	db	C_MODRM 		; 20h - mov r32, cr0-cr7
	db	C_MODRM 		; 21h - mov r32, dr0-dr7
	db	C_MODRM 		; 22h - mov cr0-cr7, r32
	db	C_MODRM 		; 23h - mov dr0-dr7, r32
	db	C_UNDEFINED		; 24h
	db	C_UNDEFINED		; 25h
	db	C_UNDEFINED		; 26h
	db	C_UNDEFINED		; 27h
	...
	db	C_NONE			; 77h - emms
	db	C_MODRM 		; 78h - vmread r/m32, r32
	db	C_MODRM 		; 79h - vmwrite r32, r/m32
	db	C_UNDEFINED		; 7Ah
	db	C_UNDEFINED		; 7Bh
	...
	db	C_GROUP+C_MODRM 	; AEh - fxsave m512byte (also fxrstor, ldmxcsr m32, stmxcsr m32, xsave, xrstor, xsaveopt, clflush m8) || [F3] rdfsbase (also rdgsbase, wrfsbase, wrgsbase, <>, {no prefix->} lfence, mfence, sfence)
	...
	db	C_MODRM 		; B9h - ud1		  (might be wrong)
	...
	db	C_GROUP+C_MODRM 	; C7h - <>		  (also cmpxchg8b m64, <>, <>, <>, <>, vmptrld m64|[66] vmclear m64|[F3] vmxon m64 || rdrand r32, vmptrst m64|[F3] vmptrst || rdseed r32)
